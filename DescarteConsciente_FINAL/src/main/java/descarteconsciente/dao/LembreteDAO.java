...
public class LembreteDAO {
    ...
    public boolean atualizar(Lembrete l) {
    String sql = "UPDATE lembretes SET medicamento=?, data_validade=? WHERE id=? AND usuario_id=?";
    try (Connection c = DatabaseConnection.getConnection();
         PreparedStatement ps = c.prepareStatement(sql)) {
        ps.setString(1, l.getMedicamento());
        ps.setDate(2, Date.valueOf(l.getDataValidade()));
        ps.setInt(3, l.getId());
        ps.setInt(4, l.getUsuarioId());
