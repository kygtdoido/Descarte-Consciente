...
public class ListaPontosPanel extends JPanel {
    ...
    private String tipoSelecionado() {
        ...
        if (index == 1) return PontoDeColeta.FARMACIA;
        if (index == 2) return PontoDeColeta.DROGARIA;
        if (index == 3) return PontoDeColeta.POSTO_SAUDE;
        if (index == 4) return PontoDeColeta.PONTO_MUNICIPAL;
        return "TODOS";
    }
    ...
    private void initComponents() {
        designBusca = new JTextField();
        designFiltro = new JComboBox<String>(new String[]{"Todos os tipos", "Farmácias", "Drogarias"});
        designLista = new JList<String>(new String[]{"Drogaria Saúde", "Farmácia Bem+", "Posto Municipal"});
        designBtnLista = new JButton("Lista");
        designBtnMapa = new JButton("Mapa");
        setLayout(null);
        add(designBusca);
        designBusca.setBounds(20, 20, 620, 38);
        add(designFiltro);
        designFiltro.setBounds(20, 75, 220, 36);
        add(designBtnLista);
        designBtnLista.setBounds(690, 75, 90, 36);
        add(designBtnMapa);
        designBtnMapa.setBounds(780, 75, 90, 36);
        add(designLista);
        designLista.setBounds(20, 135, 850, 350);
    }

    private JTextField designBusca;
    private JComboBox<String> designFiltro;
    private JList<String> designLista;
    private JButton designBtnLista;
    private JButton designBtnMapa;
}
